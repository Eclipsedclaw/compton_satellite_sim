...
void Analysis::book(G4bool isMaster) {
   ...
   man->SetFirstHistoId(1); // 设置直方图 ID 从 1 开始
    ...
    man->CreateNtupleIColumn("eventID");        // Column 0
    man->CreateNtupleIColumn("trackID");        // Column 1
    man->CreateNtupleIColumn("stepID");         // Column 2
    man->CreateNtupleIColumn("parentID");       // Column 3
    man->CreateNtupleIColumn("chamberID");      // Column 4
    // 🔹 新增 pixelID
    man->CreateNtupleIColumn("pixelID");        // Column 5
    man->CreateNtupleDColumn("x_pre");          // Column 6
    man->CreateNtupleDColumn("y_pre");          // Column 7
    man->CreateNtupleDColumn("z_pre");          // Column 8
    man->CreateNtupleDColumn("px_MeV");         // Column 9
    man->CreateNtupleDColumn("py_MeV");         // Column 10
    man->CreateNtupleDColumn("pz_MeV");         // Column 11
    man->CreateNtupleDColumn("angle_deg");      // Column 12
    man->CreateNtupleDColumn("eDep_MeV");       // Column 13
    man->CreateNtupleDColumn("kineticEnergy_MeV");// Column 14
    man->CreateNtupleSColumn("particleName");   // Column 15
    man->CreateNtupleSColumn("creatorProcess"); // Column 16
    man->CreateNtupleDColumn("time_ns");        // Column 17
    man->CreateNtupleDColumn("weight");         // Column 18
    man->CreateNtupleDColumn("Step_lenth_mm");  // Column 19
    man->CreateNtupleSColumn("Process_pre");    // Column 20
    man->CreateNtupleDColumn("x_post");         // Column 21
    man->CreateNtupleDColumn("y_post");         // Column 22
    man->CreateNtupleDColumn("z_post");         // Column 23
    man->CreateNtupleSColumn("Process_post");   // Column 24
    man->FinishNtuple();
...
void Analysis::FillNtuple(TrackerHitsCollection* hitsCollection) {
    ...
    for (G4int i = 0; i < nofHits; i++) {
        ...
        // 🔹 填充新增的 pixelID
        man->FillNtupleIColumn(5, hit->GetPixelID());
        
        // 👇 后续的所有索引号全部顺延 +1
        man->FillNtupleDColumn(6, hit->GetPos().x() / mm);
        man->FillNtupleDColumn(7, hit->GetPos().y() / mm);
        man->FillNtupleDColumn(8, hit->GetPos().z() / mm);
        man->FillNtupleDColumn(9, hit->GetMomentum().x() / MeV);
        man->FillNtupleDColumn(10, hit->GetMomentum().y() / MeV);
        man->FillNtupleDColumn(11, hit->GetMomentum().z() / MeV);
        man->FillNtupleDColumn(12, hit->GetAngle() / deg);
        man->FillNtupleDColumn(13, hit->GetEdep() / MeV);
        man->FillNtupleDColumn(14, hit->GetKineticEnergy() / MeV);
        man->FillNtupleSColumn(15, hit->GetParticleName());
        man->FillNtupleSColumn(16, hit->GetCreatorProcess());
        man->FillNtupleDColumn(17, hit->GetTime() / ns);
        man->FillNtupleDColumn(18, hit->GetWeight());
        man->FillNtupleDColumn(19, hit->GetStepLength() / mm);
        man->FillNtupleSColumn(20, hit->GetProcess());
        man->FillNtupleDColumn(21, hit->PostPos().x() / mm);
        man->FillNtupleDColumn(22, hit->PostPos().y() / mm);
        man->FillNtupleDColumn(23, hit->PostPos().z() / mm);
        man->FillNtupleSColumn(24, hit->GetpostProcess());
